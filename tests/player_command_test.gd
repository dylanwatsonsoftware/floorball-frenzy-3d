...
func _init() -> void:
	...
	if command_script == null:
		...
		return
	var command = command_script.create(12, 340, Vector2(0.8, -0.2), Vector2.RIGHT, true, 4, 7, 3, true, true)
	var packet: Dictionary = command.to_network_packet(12345, 84.0)
	for key in ["seq", "tick", "move", "facing", "shoot", "pass", "pass_seq", "switch_seq", "dash_seq"]:
		if not packet.has(key):
			fail("PlayerCommand network packet is missing %s; got %s" % [key, packet])
			return
	if not bool(packet.pass):
		fail("PlayerCommand must transmit the held Pass state so guests can charge passes")
		return
