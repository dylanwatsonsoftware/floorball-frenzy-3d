...
| 2 — shared player simulation | `PlayerMotor` drives authoritative and predicted movement; human/AI and possession speed modifiers have one shared rule; `PlayerCommand` carries movement, facing, shoot, pass, switch, dash, sequence, and tick; 30/60 FPS fixed-step equivalence test | Move the remaining authoritative actor wrapper state into the command simulation rather than mirroring its rules |
