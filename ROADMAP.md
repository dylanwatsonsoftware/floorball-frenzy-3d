...
| 4 — ball/action prediction | Explicit owner/state/generation/action metadata, sticky authoritative possession that cannot transfer through incidental overlap, possessed blade following, loss-tolerant sequenced pickup requests with explicit host accept/reject decisions, immediate predicted pass/shot, stale-snapshot rejection, and bounded host transform history for lag-compensated guest slap/pass validation; seeded pass/shot gate passes at 150 ms RTT / 2% loss | Predicted goal presentation and real-device validation |
