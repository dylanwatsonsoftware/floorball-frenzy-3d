...
| --- | --- | --- |
| 1 — diagnostics | In-game FPS/frame/RTT/jitter/loss/snapshot-age/prediction-error HUD plus sent/acknowledged command gap and browser-selected direct/TURN-relay path; deterministic network-condition profiles and seeded schedules; bounded JSON command/snapshot capture with shared-simulation replay and one-tap web download | Real-device baseline reports across the validation matrix |
| 2 — shared player simulation | `PlayerMotor.step_command_state` drives every authoritative field-player and goalkeeper wrapper as well as guest prediction/replay; human/AI, possession, and goalkeeper role modifiers have one shared rule; `PlayerCommand` carries movement, facing, shoot, pass, switch, dash, sequence, and tick; 30/60 FPS fixed-step equivalence test | Real-device handling/feel validation |
| 3 — guest reconciliation | Host acknowledgements advance only after the authoritative actor simulates the command; outstanding-command replay, timestamped remote interpolation buffers, bounded gap extrapolation/deceleration, rotation smoothing, and loss-tolerant immediate dash prediction; seeded movement, remote-buffer, and dash gates pass at 150 ms RTT / 2% loss | Real-device validation under bursty delivery |
| 4 — ball/action prediction | Explicit owner/state/generation/action metadata, sticky authoritative possession that cannot transfer through incidental overlap, possessed blade following, loss-tolerant sequenced pickup requests with explicit host accept/reject decisions, immediate predicted pass/shot, provisional guest goal presentation with clean rejection rollback, stale-snapshot rejection, and bounded host transform history for lag-compensated guest slap/pass validation; seeded pass/shot gate passes at 150 ms RTT / 2% loss | Real-device validation |
| Later 1 — shared skeleton and authored animation | Lamb and Pirate GLBs share an 18-bone humanoid armature and six authored Blender actions: idle, run, backpedal, strafe left/right, and slap shot. Editable `.blend` sources and a deterministic rebuild script are checked in. | Tune clip timing from real-device play sessions |
| Later 2 — animation integration | Every field player uses a directional `AnimationTree` locomotion blend space, an additive filtered upper-body slap layer, and two live `SkeletonIK3D` hand chains targeting the moving stick shaft. | Tune blend/IK weights from real-device play sessions |
