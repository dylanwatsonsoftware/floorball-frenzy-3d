...
| 3 — guest reconciliation | Host acknowledgements advance only after the authoritative actor simulates the command; outstanding-command replay, timestamped snapshots, remote extrapolation, rotation smoothing, and loss-tolerant immediate dash prediction; seeded movement and dash gates pass at 150 ms RTT / 2% loss | Real-device validation and explicit remote interpolation buffer under bursty delivery |
