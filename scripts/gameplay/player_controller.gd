...
func _physics_process(delta: float) -> void:
	...
	var facing_planar := input_vector
	if not is_human_controlled():
		...
		facing_planar = SquadLogicScript.tactical_facing(Vector2(global_position.x, global_position.z), input_vector, _ball.global_position, owner_team == get_team(), has_ball)
	if _shot_aim_locked:
		facing_planar = SquadLogicScript.attacking_goal_direction(get_team(), global_position)
	if not is_human_controlled():
		speed_multiplier *= SquadLogicScript.facing_movement_multiplier(input_vector, facing_planar)
	var command := {"move": input_vector, "facing": facing_planar, "dash_pressed": is_human_controlled() and Input.is_action_just_pressed("dash"), "delta": delta, "speed_multiplier": speed_multiplier}
