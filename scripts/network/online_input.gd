...
static func follow_possessed_ball(current: Vector3, blade_target: Vector3, _delta: float) -> Vector3:
	# Explicit possession is rendered from the locally animated stick. Applying
	# another smoothing spring here makes the ball trail behind every turn.
	return Vector3(blade_target.x, current.y, blade_target.z)
