...
For a repeatable 12-player animation/script benchmark, run `godot --headless --path . --script tools/profile_match_runtime.gd`. The JSON result is also saved to `/tmp/floorball-runtime-profile.json`. This isolates game-side CPU work; it does not replace foreground profiling of the exported web build on physical target phones.
